      ...
      <div className="hidden lg:flex relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col justify-between p-10 w-full">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-white font-semibold tracking-wide">{APP_NAME}</div>
              <div className="text-xs text-slate-400">{APP_TAGLINE}</div>
            </div>
          </div>

          <div className="max-w-md">
            <div className="text-[11px] tracking-[0.2em] uppercase text-blue-400 font-semibold mb-3">Enterprise ERP</div>
            ...
            <p className="text-slate-400 mt-4 text-sm leading-relaxed">
              Inventory across branches, POS, HRMS with UAE document expiry alerts, finance & compliance — all in one command center.
            </p>
          </div>

          <div className="text-[11px] text-slate-500">© 2026 GHAZA COMPUTER LLC · Dubai, UAE</div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center p-6 sm:p-12">
        <div className="w-full max-w-sm">
          ...
          <form onSubmit={submit} className="mt-8 space-y-4">
            ...
            <Button type="submit" disabled={busy} className="w-full h-11 bg-blue-600 hover:bg-blue-700 text-white" data-testid="login-submit-btn">
              {busy ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Signing in…</> : <><Lock className="w-4 h-4 mr-2" /> Sign in securely</>}
            </Button>
          </form>
        </div>
      </div>
