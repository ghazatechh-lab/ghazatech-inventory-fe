...
function KpiCard({
  ...
  return (
    ...
    >
      <div
        className={`absolute -top-8 -right-8 w-32 h-32 bg-gradient-radial rounded-full bg-gradient-to-br ${tintMap[tint]} blur-2xl opacity-70`}
      />
      <div className="relative">
        ...
        {delta !== undefined && (
          <div
            className={`mt-1 flex items-center text-[11px] ${positive ? "text-emerald-400" : "text-red-400"}`}
          >
            {positive ? (
    ...
    queryFn: async () =>
      unwrap(
        await api.get("/dashboard/", {
          params: branchOverride ? { branch_id: branchOverride } : {},
        }),
      ),
  });
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <KpiCard
          ...
          icon={TrendingUp}
          tint="green"
          testId="kpi-sales-month"
        />
        <KpiCard
          title="Purchases this month"
          value={k.purchases_month}
          delta={-3.1}
          icon={ShoppingCart}
          tint="amber"
          testId="kpi-purchases-month"
        />
        <KpiCard
        ...
        <KpiCard
          ...
          value={k.pending_quotations}
          icon={FileText}
          tint="blue"
          isCurrency={false}
          testId="kpi-pending-quotations"
        />
      </div>
        ...
        <div className="card-surface p-5">
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  data={(data?.sales_by_category || []).slice(0, 6)}
                  dataKey="value"
                  nameKey="category"
                  innerRadius={45}
                  outerRadius={80}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="card-surface p-5">
          ...
          <div className="space-y-2">
            {(data?.top_products || []).length === 0 && (
              <EmptyState
                title="No sales yet"
                description="Data will appear once you record sales."
              />
            )}
            {(data?.top_products || []).map((p, i) => (
              <div
                key={i}
                className="flex items-center justify-between p-2 rounded-md hover:bg-white/[0.02]"
              >
                ...
                <div className="font-numeric text-xs text-slate-300">
                  {formatAED(p.value)}
                </div>
              </div>
            ))}
        ...
        <div className="card-surface p-5">
          ...
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {(data?.doc_expiry_alerts || []).length === 0 && (
            ...
            {(data?.doc_expiry_alerts || []).map((a, i) => (
              <Link
                to={`/hrms/employees/${a.employee.id}`}
                key={i}
                className="flex items-center justify-between p-2 rounded-md hover:bg-white/[0.02]"
              >
                ...
                <StatusBadge
                  ...
                  label={a.days_left < 0 ? "Expired" : `${a.days_left}d`}
                />
              </Link>
            ))}
          </div>
          ...
          <div className="space-y-2">
            {(data?.recent_invoices || []).map((inv) => (
              <Link
                ...
                key={inv.id}
                className="flex items-center justify-between p-2.5 rounded-lg hover:bg-white/[0.02]"
              >
                <div className="min-w-0">
                  <div className="text-sm text-slate-100 font-numeric">
                    {inv.invoice_number}
                  </div>
                ...
                <div className="text-right">
                  <div className="text-sm font-numeric text-white">
                    {formatAED(inv.total)}
                  </div>
            ...
            {(data?.low_stock_products || []).map((s) => (
              ...
              >
                ...
                <div className="text-right shrink-0">
                  <div className="font-numeric text-sm text-white">
                    {s.quantity}
                  </div>
                  <div className="text-[10px] text-slate-500">
