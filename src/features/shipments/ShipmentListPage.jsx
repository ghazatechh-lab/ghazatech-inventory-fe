...
export default function ShipmentListPage() {
  ...
  return (
    <div>
      <PageHeader title="Shipments" subtitle="Deliveries to customers"
        actions={<Button asChild className="bg-blue-600 hover:bg-blue-700"><Link to="/shipments/new"><Plus className="w-4 h-4 mr-1.5" /> New shipment</Link></Button>} />
      <DataTable
        columns={[
          { key: "shipment_number", header: "Shipment #", cell: (r) => <Link to={`/shipments/${r.id}`} className="font-numeric text-blue-400">{r.shipment_number}</Link> },
          ...
          { key: "expected", header: "Expected", cell: (r) => <DateText value={r.expected_date} /> },
