...
import { Plus, Barcode as BarcodeIcon } from "lucide-react";
import {
  Select,
  SelectContent,
  SelectItem,
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        ...
        <Select
          value={getFilter("brand")}
          onValueChange={(v) => setFilter("brand", v === "all" ? "" : v)}
        >
          <SelectTrigger className="w-40 bg-white/[0.02] border-white/10 h-9 text-sm">
        ...
        >
          <SelectTrigger className="w-44 bg-white/[0.02] border-white/10 h-9 text-sm">
        ...
        columns={[
          {
            ...
            cell: (r) => (
              <Link
                to={`/inventory/products/${r.id}`}
                className="flex items-center gap-3"
              >
                <img
                  src={r.product_image}
                  alt=""
                  className="w-9 h-9 rounded-md object-cover ring-1 ring-white/5"
                />
                <div>
                  <div className="text-slate-100">{r.product_name}</div>
                  <div className="text-[10px] text-slate-500 flex items-center gap-2 mt-0.5">
                    <BarcodeIcon className="w-3 h-3" /> {r.barcode}
                  </div>
                </div>
              </Link>
            ),
          },
          {
            ...
            header: "SKU",
            cell: (r) => (
              <span className="font-numeric text-slate-300">{r.sku}</span>
            ),
          },
          ...
          { key: "category", header: "Category" },
          {
            ...
            header: "Purchase",
            align: "right",
            cell: (r) => <CurrencyText value={r.purchase_price} />,
          },
          {
            key: "retail_price",
            header: "Retail",
            align: "right",
            cell: (r) => (
              <CurrencyText value={r.retail_price} className="text-slate-100" />
            ),
          },
          {
            key: "rack_location",
          ...
          },
          {
            ...
            cell: (r) => (
              <ListingRowActions
                viewTo={`/inventory/products/${r.id}`}
                deleteUrl={`/products/${r.id}/`}
                queryKey="products"
                itemLabel={r.product_name || r.sku || "product"}
              />
            ),
          },
        ]}
